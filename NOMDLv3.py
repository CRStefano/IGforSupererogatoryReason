...
def discretize_column(rows: List[Dict], col: str, analysis: Dict) -> List[str]:
    ...
    # SE ≤ 10 valori unici → CATEGORICA → ritorna as-is (senza conversione!)
    if n_unique <= 10:
...
def analyze_columns(rows: List[Dict], target_col: str, threshold: Any,
    ...
    for col in reason_cols:
        try:
            ...
            # SE ≤ 10 valori unici → CATEGORICA → skippa analisi numerica
            if n_unique <= 10:
                analysis[col] = {
...
def greedy_ig_selection(target_values: List[str],
    ...
    tau_min = max(0.10, tau)  # Almeno 10%
...
def compute_conditional_ig(target_values: List[str], 
    ...
    if not selected_features:
        ...
        H_target = calculate_entropy(target_values)
        H_cond = compute_conditional_entropy(target_values, new_feature)
        IG = H_target - H_cond
    ...
    # H(Y | Z)
    H_Y_given_Z = compute_conditional_entropy(target_values, combined_context)
    ...
    # H(Y | X, Z)
    H_Y_given_XZ = compute_conditional_entropy(target_values, combined_with_new)
...
def compute_conditional_entropy(target_values: List[str], 
                               context_values: List[str]) -> float:
    """
    Calcola H(Y | X): entropia di Y dato X.
    """
    # Raggruppa target per valore di context
    context_groups = {}
    for i, ctx_val in enumerate(context_values):
        if ctx_val not in context_groups:
            context_groups[ctx_val] = []
        context_groups[ctx_val].append(target_values[i])
    
    # Entropia condizionale pesata
    total = len(target_values)
    H_conditional = 0.0
    
    for ctx_val, target_subset in context_groups.items():
        weight = len(target_subset) / total
        H_subset = calculate_entropy(target_subset)
